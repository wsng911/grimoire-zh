		...
		<div class="my-4 flex max-w-2xl flex-col gap-4 text-center">
			...
			{#if env?.PUBLIC_SIGNUP_DISABLED === 'true'}
				<p class="text-lg">
					...
					<strong><a href="/signup" class="link">创建您的第一个用户</a></strong> 开始使用 Grimoire。
				</p>
			{:else}
				<p class="text-lg">
					Please go ahead and <strong
						><a href="/signup" class="link text-secondary">创建您的第一个用户</a></strong> (Root
					Admin).
				</p>
			{/if}
			<p class="text-lg">
				或者，如果您使用的是<span class="text-warning">旧版本</span>（使用
				PocketBase 存储数据），可以使用
				<strong><a href="/migrate" class="link text-secondary">迁移工具</a></strong>轻松
				导入您的所有数据！
			</p>
		</div>
	</div>
